...
import { cartLineUnitPrice } from '../../src/lib/cartLine';
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 16 }}>
        {lines.map((line) => {
          const optionsSubtitle = (line.selections ?? []).flatMap((s) => s.choiceNames).join(', ');
          ...
          return (
            <View key={line.lineId} style={styles.row}>
              ...
              <View style={{ flex: 1, gap: 2 }}>
                ...
                {optionsSubtitle ? (
                  <Text style={styles.rowOptions} numberOfLines={2}>
                    {optionsSubtitle}
                  </Text>
                ) : null}
                {line.notes ? (
                  <Text style={styles.rowOptions} numberOfLines={1}>
                    Note: {line.notes}
                  </Text>
                ) : null}
                <Text style={styles.rowPrice}>${unitPrice.toFixed(2)}</Text>
              </View>
              <View style={styles.qtyControl}>
                <Pressable onPress={() => decrementLine(line.lineId)} hitSlop={6}>
                  <Ionicons name="remove-circle" size={24} color={colors.orange} />
                </Pressable>
                <Text style={styles.qtyText}>{line.quantity}</Text>
                <Pressable onPress={() => incrementLine(line.lineId)} hitSlop={6}>
...
const makeStyles = (colors: ThemePalette) =>
  ...
  rowOptions: { fontSize: 11.5, color: colors.textLight },
