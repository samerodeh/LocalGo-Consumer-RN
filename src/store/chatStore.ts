...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  configure: async (orderId) => {
    ...
    const { data } = await sb
      .from('messages')
      .select('id, order_id, sender_id, body, image_url, created_at, read_at')
      .eq('order_id', orderId)
  ...
  send: async (body) => {
    ...
    if (!text || !orderId || !myUid || !supabase) return;

    set({ sending: true });
    try {
      const { data, error } = await supabase
        .from('messages')
        .insert({ order_id: orderId, sender_id: myUid, body: text })
        .select('id, order_id, sender_id, body, image_url, created_at, read_at')
        .single();
      ...
      void notifyPush(orderId, 'new_message', { senderId: myUid, preview: text });
    } catch (err) {
