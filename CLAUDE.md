...
- **expo-notifications** — local (in-app, sound-on-everywhere) + remote (physical-EAS-only) push, see below
- **expo-image-picker**, **expo-audio** — chat photo/voice attachments (`src/lib/chatMedia.ts`)
...
- **Two notification layers** — remote push (`src/lib/notifications.ts::registerForPushNotifications`,
  backend `POST /notify`) only reaches physical EAS builds (Expo Go/simulators/web can't mint a
  push token). `ensureNotificationPermission()` + `presentLocalNotification()` in the same file add a
  **local** layer that fires the same alerts *with sound* everywhere, including Expo Go and the web
  preview — wired into `useOrderTracking.ts` (accepted/picked_up/delivered transitions) and, on the
  driver side, `useOrderFeed.ts` (new order lands in the feed).
- **`src/store/usePendingOrders.ts`** — watches the customer's own just-placed, still-unaccepted
  orders and drives the "Finding you a driver…" card (`src/components/PendingOrderStack.tsx`,
  mounted on Home above `OrderTrackingStack`). After `PATIENCE_MS` (60s) still unclaimed, it flips to
  a "we're a new business, thanks for your patience" reassurance and fires one local notification —
  there was previously no live feedback between checkout and a driver accepting.
- **Chat photos + voice notes** — `messages` gained `audio_url`/`audio_duration_sec` columns (schema
  in `../LocalGODriverRN/supabase/schema.sql`, already applied live) alongside the existing
  `image_url`. `src/lib/chatMedia.ts` picks/uploads a photo to the public `order-photos` bucket
  (same bucket the driver's proof-photos already use) or a recorded voice note to the new public
  `chat-audio` bucket; `chatStore.send(body, { imageUrl?, audioUrl?, audioDurationSec? })` accepts an
  optional attachment. Recording/playback uses `expo-audio` (`VoiceRecordButton.tsx` /
  `ChatMessageBubble.tsx`'s `VoicePlayer`) — both participants (customer and driver) can send either
  kind from either app's chat composer.
...
**Al Taib** (`al-taib`, 73 items) and **Château Kabab** (`chateau-kabab`, 77 items) both have real
menus (`hasMenu: true`), live in `src/data/menu.ts` with remote product photos. Other partners
would render a "coming soon" state.

- **Per-item customization** — every item's real choosable preferences (size, add-ons, quantity
  tiers, protein/skewer choices, upsell "recommended drinks/sides/desserts" groups) were scraped
  and verified directly from altaib.ca and chateaukabab.com (both BetterResto-powered storefronts)
  and modeled as `MenuOptionGroup[]` on `MenuItem` (`src/types/index.ts`). Shared upsell groups
  live once in `src/data/menuOptions.ts`; per-item groups are keyed by menu item id in
  `src/data/menuItemOptions.ts` and attached to `menuItems`/`chateauKababMenuItems` at module load
  in `menu.ts`. Items with no option groups (plain drinks, single-note sides) keep the fast inline
  +/- stepper; items with groups open `src/components/ItemOptionsSheet.tsx` instead.
- **Cart lines are customization-aware** — `CartLine.lineId` (not `item.id`) is the identity key, so
  two different configurations of the same item are separate lines (`src/store/cartStore.ts`).
  `src/lib/cartLine.ts` (`cartLineUnitPrice`, `cartLineDisplayName`) is the single source of truth
  for a customized line's price and human-readable summary — used by the cart screen, order
  records (`ordersStore.record()`), and the driver-facing dispatch payload (`dispatch.ts`) alike.
